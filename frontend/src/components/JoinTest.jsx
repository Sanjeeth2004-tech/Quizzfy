...
function JoinTest() {
  ...
  //     navigate(`/quiz-Custom/${encodeURIComponent(foundQuiz.name)}`, {
  //       state: {
  //         isCustom: true,
  //         joinMode: true,
  //         code: foundQuiz.code,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // const response = await axios.get(`/api/courses/${encodeURIComponent(testCode.trim())}`);
      // const quiz = response.data;
  
      navigate(`/quiz/${encodeURIComponent(testCode.trim())}`, {
        state: {
          isCustom: true,
          joinMode: true,
          code: testCode.trim(),
          questions: testCode.trim(),
        }
      });
    } catch (err) {
      setError('Invalid test code. Please try again.');
